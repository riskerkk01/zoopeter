...
var adbrixre = Vue.component('adbrixre', {
	template : '#adbrixre',
	data : function() {  
		return {
			totalCount : 0,
	...
	created : function() {
		...
		console.log("adbrixre created");
	},
	mounted : function() {
		console.log('adbrixre');
		//this.getList();
	...
	methods : {
		...
		updateform : function(data) {
			var self = this
		    axios.get('/api/adbrixre/'+data.seq).then(res => {
			        var tmp = res.data;
		...
		deleteact : function() {
			...
			axios.post('/api/adbrixre/delete/'+self.requestupdate.seq, self.requestupdate).then(res => {
		...
		getList : function() {
		      var self = this
		      axios.post('/api/adbrixre/list', self.request).then(res => {
        ...
        exportExcel() {
		    ...
		    axios.post('/api/adbrixre/list', tmp).then(res => {
		...
		downExcel() {
		      ...
		      //console.log(data)
		      var data = XLSX.writeFile(wb, today+'_adbrixre.xlsx', {compression:true, bookType:'xlsx'})
		      /*
